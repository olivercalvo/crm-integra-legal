...
**Deploy log (18/09/2026):** merge `--no-ff` a `main` = `24b227a`; push de Oliver; deploy de
producción `2yf7q4gyc` Ready, alias `crm-integra-legal.vercel.app`. Deploy anterior (destino del
Instant Rollback): `4jogydgyk`. **Verificación post-deploy pendiente:** la primera factura de
reembolso real la emite el bufete; se confirma que el CUFE empiece con `FE09` y el estado sea
"Autorizada DGI". No se probó contra producción.
