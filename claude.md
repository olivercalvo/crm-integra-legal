...
- **`/finanzas/cobros` es de admin y abogada** (mismo reparto que Facturas). El contador baja el
  PDF y reversa desde el detalle de la factura. Preguntar a Josuarth si quiere ver el listado
  está pendiente (`task_plan.md`); si sí, mover `nav-config.ts` y `route-access.ts` juntos.
