...
export async function POST(
  ...
  try {
    ...
    if (payment.receipt_url) {
      await admin.storage.from("documents").remove([payment.receipt_url]);
    }

    const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, "_");
    const storagePath = `${profile.tenant_id}/pagos/${payment.case_id}/${paymentId}/${Date.now()}_${safeName}`;
    ...
    await admin.from("audit_log").insert({
      ...
      action: "update",
      field: "receipt",
      old_value: payment.receipt_url ?? null,
...
export async function DELETE(
  ...
  try {
    ...
    if (!payment.receipt_url) {
      return NextResponse.json({ error: "Este pago no tiene recibo adjunto" }, { status: 400 });
    }
