...
import { AsistenteHome } from "@/components/dashboards/asistente-home";

export default async function LegalDashboard() {
  const ctx = await getAuthenticatedContext();
  // Asistente ve un dashboard distinto (sus casos y tareas asignadas).
  if (ctx.userRole === "asistente") {
    return <AsistenteHome />;
  }
  ...
  // Misma lógica que /legal/pendientes (ownedRaw): cargar TODOS y filtrar status en JS
  ...
  // Misma lógica que /legal/pendientes (assignedRaw): cargar TODOS y filtrar status en JS.
  ...
  const stats = [
    ...
    },
    {
      ...
      href: "/legal/casos?status=en-tramite",
    },
    {
      label: "Cerrados",
      ...
      href: "/legal/casos?status=cerrado",
    },
    {
          ...
          <Card className="border-red-200 bg-red-50/50">
            ...
            <CardContent className="space-y-2">
              {overdueCases.slice(0, 3).map((c) => (
                <Link key={c.id} href={`/legal/casos/${c.id}`} className="block">
          ...
          <Card className="border-red-200">
            ...
            <CardContent className="space-y-2">
              {casesInRed.slice(0, 4).map((c) => {
                ...
                return (
                  <Link key={c.id} href={`/legal/casos/${c.id}?tab=gastos`} className="block">
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <Link href="/legal/pendientes" className="text-xs font-medium text-integra-navy hover:underline">
              Ver todos →
            </Link>
          </div>
        </CardHeader>
